...
And, $$\mbox{E} (\sigma_g^2 | s_g^2) = \frac{d_g s_g^2 + d_0 s_0^2} {d_g + d_0 - 2}$$
...
Now we compute the null joint distribution of $\hat{\beta}_{gj}$ and $s_g^2$, 
$$f(\hat{\beta}_{gj}, s_g^2 | \beta = 0) = \int f(\hat{\beta}_{gj} | s_g^2, \beta = 0) f(s_g^2 | \sigma_g^2) f(\sigma_g^2) \, d(\sigma_g^2) $$
...
which integrates to:
$$f(\hat{\beta}_{gj}, s_g^2 | \beta = 0) \\= 
(1/{2 v_{gj}}^{1/2}) (d_0 s_0^2 / 2)^{d_0/2} (d_g/2)^{d_g/2} s_g^{2(d_g/2 - 1)} \frac{\Gamma(1/2 + d_0/2 + d_g/2)} {\Gamma(d_0/2) \Gamma(d_g/2)} (\frac{\hat{\beta}_{gj}^2 / v_{gj} + d_0 s_0^2 + d_g s_g^2}{2})^{-(1 + d_0 + d_g)/2}$$
Clearly $\hat{\beta}_{gj}$ and $s_g^2$ is not independent.